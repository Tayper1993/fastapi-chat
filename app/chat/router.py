...
async def get_chat_page(request: Request, user_data: User = Depends(get_current_user)):
    ...
    return templates.TemplateResponse('chat.html', {'request': request, 'user': user_data, 'users_all': user_all})
...
async def send_message(message: MessageCreate, current_user: User = Depends(get_current_user)):
    ...
    await MessagesDAO.add(sender_id=current_user.id, content=message.content, recipient_id=message.recipient_id)
